...
// The visible annotation appended to a legacy-standalone row (color codes have
// zero display width, so only this bare text counts toward the border math).
const FORMKIT_MARK = "  ⇢ auro-formkit";

/**
 * Render the "components behind latest" notice as a bold, bordered banner with
 * an aligned version table and ready-to-run guidance. Colors degrade
 ...
 *
 * A **legacy standalone** form package (one `auro-formkit` now ships) is not advised
 * to update to `@latest` — that pushes the consumer deeper into the deprecated
 * package. Its row is marked `⇢ auro-formkit` and it appears under a distinct
 * "Migrate to auro-formkit" block instead of the `npm install @latest` command.
 */
export function renderOutdatedBanner(outdated: OutdatedComponent[]): string {
  ...
  const legacy = outdated.filter((o) => isLegacyFormkitPackage(o.pkg));
  const normal = outdated.filter((o) => !isLegacyFormkitPackage(o.pkg));
  ...
  const isLegacy = (o: OutdatedComponent): boolean =>
    isLegacyFormkitPackage(o.pkg);
  const rows = outdated.map((o) => {
    const row = `  ${chalk.bold(o.pkg.padEnd(pkgWidth))}  ${chalk.dim(
      o.installed.padStart(installedWidth),
    )} ${chalk.dim("→")} ${chalk.green.bold(o.latest)}`;
    return isLegacy(o) ? `${row}${chalk.yellow(FORMKIT_MARK)}` : row;
  });

  // Guidance: standard packages update to @latest; legacy standalones migrate to
  // the formkit subpath instead. Either block is omitted when it has no members.
  const guidance: string[] = [];
  if (normal.length > 0) {
    const updateCmd = `npm install ${normal
      .map((o) => `${o.pkg}@latest`)
      .join(" ")}`;
    guidance.push(
      "",
      chalk.dim("  Update to latest with:"),
  ...
  if (legacy.length > 0) {
    guidance.push(
      "",
      chalk.dim("  Migrate to auro-formkit (run `auro init`):"),
    );
    for (const o of legacy) {
      guidance.push(
        `  ${chalk.dim(o.pkg)} ${chalk.dim("→")} ${chalk.cyan(formkitSubpathFor(o.pkg))}`,
      );
    }
  }
  ...
  const visibleWidths = [
    ...
    heading.length + 1,
    ...outdated.map((o) => {
      const base = 2 + pkgWidth + 2 + installedWidth + 3 + o.latest.length;
      return isLegacy(o) ? base + FORMKIT_MARK.length : base;
    }),
  ];
